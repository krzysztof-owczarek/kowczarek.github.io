...
Recently, I have been working on performing load tests on the orchestrator application we have created to introduce a new service to our portfolio. The task was very important because the application had to perform well under significant traffic expected to appear in the shape of significant spikes.

For the needs of this article, I will refer to the application as `the orchestrator`, because its main task was to orchestrate synchronous calls to other systems and combine the result into a successful response.
...
Introduce mocks for external services and configure your application to use them instead of the real services. Remember to introduce artificial latencies for the responses. Set their values to be similar to the average response time of the mocked service.  To make the mocks behave even more realistically, make the latencies vary a little for each call - they could take a value from a range of:
...
Introduce external integrations one by one, rerunning the load tests each time, making sure the test is still passing. If any problems occur, resolve them immediately in cooperation with the owners of the service in question. Rerun the test after each change.

> At this point, you may feel like finding bottlenecks in someone’s services and forcing them to work on providing a robust solution is a rude thing to do, but you are mistaken!  Finding such problems at the early stage, especially before going live with an important service, gives the team time to prepare a fix without haste, helps them build up knowledge and trust in their service, and saves all parties from busy on-calls in the future.
...
- **(hard)** organize your testing - do not change many things at once, focus on a single configuration value, rerun the load tests every time, observe the monitoring and take notes, 
...
- using a static deployment for your application makes your cluster reserve resources on available nodes upfront. When HPA starts to scale out pods, the cluster allocates a significant amount of resources on available nodes. It may happen that existing nodes are not enough and the whole cluster will have to scale out too.  As a result, the load test **will not pass** as it usually has a huge negative effect on the performance of the service,
...
  Combining this unaccounted-for CPU usage with a constant flow of incoming requests, the performance will decrease with time, until your app will finally start to time out on healthiness/readiness probes and the pod will be killed.

  There will be, of course, a new pod spawned in its place, but it will probably share the same fate.  Increasing CPU assignment in your deployment configuration may help you mitigate this issue. It will also leave you with over-provisioned CPU just after JVM warms up, as additional cycles will not be used most of the time. 
...
What is not a piece of common knowledge is that CPU request configuration guarantees that your pod will be assigned the **requested** CPU cycles (as long as there are physical CPU resources available on the cluster). Therefore not using a **limit** configuration does not save the others from being robbed from their CPU cycles by your greedy deployment.
